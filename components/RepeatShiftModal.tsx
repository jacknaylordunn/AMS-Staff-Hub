...
const RepeatShiftModal: React.FC<RepeatShiftModalProps> = ({ isOpen, onClose, shift, onSave }) => {
    ...
    const handleSave = async () => {
        ...
        const { id, ...baseShift } = shift;
        
        const startDate = shift.start.toDate();
        const endDate = new Date(untilDate + 'T23:59:59Z'); // Use end of day for comparison
        ...
        if (repeatType === 'weekly') {
            ...
            while(currentDate <= endDate) {
                ...
                // If the day doesn't exist in the month (e.g. 31st), use the last day
        ...
        try {
            if (newShifts.length > 0) {
                ...
                showToast("No future shifts to create in the selected range.", "info");
            }
            onSave();
            onClose();
        } catch (e) {
